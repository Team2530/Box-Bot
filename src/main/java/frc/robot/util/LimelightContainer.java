...
public class LimelightContainer {
  ...
  public static void estimateSimOdometry(){
    ...
    for (Limelight limelight : limelights) {
      ...
      if (!doRejectUpdate) {
        SmartDashboard.putString("Simulated Pos", mt2.pose.toString()+COUNTER);
        COUNTER++;
      }
    }
  }

  public static void estimateMT2Odometry(SwerveDrivePoseEstimator poseEstimator, ChassisSpeeds speeds, AHRS navx) {
    for (Limelight limelight : limelights) {
      boolean doRejectUpdate = false;
